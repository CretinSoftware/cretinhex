...
ARN ARN_filtrerCondition(ARN arn1, ARNElementEval estValide){
	ARN resultat = ARN_init(arn1->eval);
	ABRIterateur it = ABRIterateur_init(arn1);
	for (it = ABRIterateur_debut(it); ! ABRIterateur_fin(it); it = ABRIterateur_avancer(it))
		if (estValide(ABRIterateur_valeur(it)))
			resultat = ARN_inserer(resultat, ABRIterateur_valeur(it), NULL);
	ABRIterateur_free(&it);
	return resultat;
}
...
ARN ARN_exfiltrerCondition(ARN arn1, ARNElementEval estInvalide){
	ARN resultat = ARN_init(arn1->eval);
	ABRIterateur it = ABRIterateur_init(arn1);
	for (it = ABRIterateur_debut(it); ! ABRIterateur_fin(it); it = ABRIterateur_avancer(it))
		if ( ! estInvalide(ABRIterateur_valeur(it)))
			resultat = ARN_inserer(resultat, ABRIterateur_valeur(it), NULL);
	ABRIterateur_free(&it);
	return resultat;
}
