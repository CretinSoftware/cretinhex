 ...
 * \param   noeudsDuJoueur tous les noeuds du joueur dans un ARN
 ...
 */
ARN noeudsRelies(GrapheNoeud noeud, ARN noeudsDuJoueur, int ponts){
	...
	for (it = LDCIterateur_debut(it); ! LDCIterateur_fin(it); it = LDCIterateur_avancer(it)){
		...
		if (! ARN_chercher(resultat, voisin)){
			...
			if (ARN_chercher(noeudsDuJoueur, voisin))
	...
	while (ponts && modifiesCeTour){
		...
		for (at = ABRIterateur_debut(at); ! ABRIterateur_fin(at); at = ABRIterateur_avancer(at)){
			...
			tmp = ARN_filtrer(tmp3, noeudsDuJoueur);
 ...
 */
LDC noeudsSuivants(GrapheHex g, LDC ldc, ARN noeudsDuJoueur, int ponts){
		...
		for (it2 = LDCIterateur_debut(it2); ! LDCIterateur_fin(it2); it2 = LDCIterateur_avancer(it2)){
			...
			if (! ARN_chercher(coucheN,        voisin) && \
				...
				if (GHElement_valeur(GrapheNoeud_obtenirElement(voisin)) == J0){
					...
					tmp  = noeudsRelies(voisin, noeudsDuJoueur, ponts);
...
LDC GrapheHex_noeudsAccessiblesEnNCoups(GrapheHex g, GrapheNoeud noeud, int n, Joueur j, int ponts){
	...
	ARN noeudsDuJoueur = ARN_fromLDC(tmp, ARNElement_adresse);
	LDC_free(&tmp);
	
	ldc = LDC_init();
	ldc = LDC_inserer(ldc, 0, noeudsRelies(noeud, noeudsDuJoueur, ponts), (LDCElementFree) ARN_free);
	...
	while (LDC_taille(ldc) < n + 1)
		ldc = noeudsSuivants(g, ldc, noeudsDuJoueur, ponts);
	
	ARN_free(&noeudsDuJoueur);
...
int GrapheHex_distanceMini(GrapheHex g, GrapheNoeud n1, GrapheNoeud n2, Joueur j, int ponts){
	...
	ldc = LDC_init();
	ldc = LDC_insererElement(ldc, 0, noeudsRelies(n1, noeudsDuJoueur, ponts), (LDCElementFree) ARN_free);
